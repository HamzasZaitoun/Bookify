document.getElementById("paymentForm").addEventListener("submit", function (event) {
    ...
    if (!validateCardNumber(cardNumber)) {
        displayMessage("cardNumberError", "Please enter a valid card number.", "error");
        valid = false;
    }
    ...
    if (!validateExpiration(expiration)) {
        displayMessage("expirationError", "Please enter a valid expiration date (MM/YY).", "error");
        valid = false;
    }
    ...
    if (valid) {
        ...
        // Empty cart section (assuming you have a method to clear it)
...
function validateExpiration(expiration) {
    ...
    return expPattern.test(expiration) && (year > currentYear || (year === currentYear && month >= currentMonth));
}
